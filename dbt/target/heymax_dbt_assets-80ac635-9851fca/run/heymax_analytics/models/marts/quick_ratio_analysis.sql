    ...
    create or replace table `heymax-interview`.`heymax_data`.`quick_ratio_analysis`
...
WITH base_growth_data AS (
    SELECT * FROM `heymax-interview`.`heymax_data`.`social_capital_growth_accounting`
),
...
platform_weekly_analysis AS (
    SELECT 
        ...
        COUNT(DISTINCT CASE 
            WHEN event_date = (
                ...
                FROM `heymax-interview`.`heymax_data`.`fct_events` f2 
                WHERE f2.user_id = f1.user_id
                AND f2.event_type IN ('reward_search', 'miles_earned', 'miles_redeemed')
            ) THEN user_id 
    ...
    WHERE f1.event_type IN ('reward_search', 'miles_earned', 'miles_redeemed')
...
utm_growth_quality AS (
    ...
    AND f1.event_type IN ('reward_search', 'miles_earned', 'miles_redeemed')
