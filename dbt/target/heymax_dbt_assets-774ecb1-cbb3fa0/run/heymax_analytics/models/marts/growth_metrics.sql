    ...
    create or replace table `heymax-interview`.`heymax_data`.`growth_metrics`
...
churned_users AS (
    SELECT today.event_date as today_date
        , today.user_id as today_user_id
        , yesterday.event_date as yesterday_date
        , yesterday.user_id as yesterday_user_id
        , CASE WHEN today.user_id is null then 1 else 0 end as churn
        , DATE_ADD(yesterday.event_date, INTERVAL 1 DAY) as churn_date
    FROM daily_users AS today
    RIGHT JOIN daily_users AS yesterday
        ON today.user_id = yesterday.user_id
        AND today.event_date = DATE_ADD(yesterday.event_date, INTERVAL 1 DAY)
    ORDER BY yesterday_user_id asc, yesterday_date asc
),
