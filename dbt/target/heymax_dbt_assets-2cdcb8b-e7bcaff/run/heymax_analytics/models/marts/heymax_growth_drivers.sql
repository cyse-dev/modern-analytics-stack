    ...
    create or replace table `heymax-interview`.`heymax_analytics`.`heymax_growth_drivers`
...
daily_miles_economy AS (
    SELECT 
        ...
        SUM(CASE WHEN event_type = 'miles_earned' THEN unique_users ELSE 0 END) as users_earning_miles,
        SUM(CASE WHEN event_type = 'miles_redeemed' THEN unique_users ELSE 0 END) as users_redeeming_miles,
        SUM(CASE WHEN event_type = 'miles_earned' THEN event_count ELSE 0 END) as miles_earning_events,
        SUM(CASE WHEN event_type = 'miles_redeemed' THEN event_count ELSE 0 END) as miles_redemption_events
...
daily_social_engagement AS (
    SELECT 
        event_date,
        SUM(CASE WHEN event_type = 'share' THEN event_count ELSE 0 END) as daily_shares,
        SUM(CASE WHEN event_type = 'like' THEN event_count ELSE 0 END) as daily_likes,
        SUM(CASE WHEN event_type = 'reward_search' THEN event_count ELSE 0 END) as daily_searches,
        SUM(CASE WHEN event_type = 'share' THEN unique_users ELSE 0 END) as users_sharing,
        SUM(CASE WHEN event_type = 'like' THEN unique_users ELSE 0 END) as users_liking,
        SUM(CASE WHEN event_type = 'reward_search' THEN unique_users ELSE 0 END) as users_searching
    FROM daily_event_summary
    GROUP BY event_date
),
...
daily_platform_metrics AS (
    SELECT 
        event_date,
        SUM(CASE WHEN platform = 'iOS' THEN unique_users ELSE 0 END) as ios_users,
        SUM(CASE WHEN platform = 'Android' THEN unique_users ELSE 0 END) as android_users,
        SUM(CASE WHEN platform = 'Web' THEN unique_users ELSE 0 END) as web_users,
        SUM(CASE WHEN platform = 'iOS' THEN event_count ELSE 0 END) as ios_events,
        SUM(CASE WHEN platform = 'Android' THEN event_count ELSE 0 END) as android_events,
        SUM(CASE WHEN platform = 'Web' THEN event_count ELSE 0 END) as web_events
    FROM daily_event_summary
    GROUP BY event_date
),
    ...
    SELECT 
        ...
        COUNT(DISTINCT CASE 
            WHEN utm_source = 'organic' AND event_date = (
                SELECT MIN(event_date) FROM `heymax-interview`.`heymax_analytics`.`fct_events` f2 
